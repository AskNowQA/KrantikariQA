...
def dataset():
    ...
    for data_point in train_data:
        try:
            train.append([[embeddings_interface.vocabularize(nlutils.tokenize(data_point[u'corrected_question']),
                                                             _embedding="glove")], [len(data_point[u'path'])]])
        except:
    ...
    for data_point in test_data:
        try:
            test.append([[embeddings_interface.vocabularize(nlutils.tokenize(data_point[u'corrected_question']),
                                                            _embedding="glove")], [len(data_point[u'path'])]])
        except:
            continue
...
for i in xrange(len(cnn_model_predict)):
    if np.argmax(cnn_model_predict[i]) == np.argmax(y_test[i]) or np.argmax(rnn_model_predict[i]) == np.argmax(
            y_test[i]):
        result = result + 1
