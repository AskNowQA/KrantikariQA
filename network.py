...
class BiLstmDenseDot:
    ...
    def _train_pairwise_(self, data, optimizer, loss_fn, device):
        ...
        ques_batch = self.dense(ques_batch[-1])
        pos_batch = self.dense(pos_batch[-1])
        neg_batch = self.dense(neg_batch[-1])
    ...
    def _train_pointwise_(self, data, optimizer, loss_fn, device):
        ...
        # Calculating dot score
        score = score = torch.sum(ques_batch[-1] * pos_batch[-1], -1)
    ...
    def predict(self, ques, paths, device):
        ...
        with torch.no_grad():
            ...
            # Dense
            question = self.dense(question[-1])
            paths = self.dense(paths[-1])
